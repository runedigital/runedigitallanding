...
// WE REMOVED THE GLOBAL CANVAS IMPORTS HERE
...
const ASSETS = {
  ...
  'MERCURY':     { tier: 'MAT CLASS',  id: '006', price: '3,500 USD', licensePrice: '$499', defaultColor: '#ffffff', stripeLink: 'https://buy.stripe.com/aFa14p2K078q1Jm2LX6Ri06' }, // <--- COMMA ADDED HERE
...
// --- ACQUISITION MATRIX ---
const AcquisitionMatrix = ({ mode, onClose, onConfigTier2, onConfigTier3, isModified, onResetRequest }) => {
  const data = ASSETS[mode]
  const Card = ({ title, price, features, color, link, action, recommended, disabled, warning }) => (
    <div style={{ border: `1px solid ${disabled ? '#333' : color}`, background: recommended ? 'rgba(20,20,20,0.95)' : 'rgba(0,0,0,0.95)', padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '480px', width: '320px', transition: 'all 0.3s ease', boxShadow: recommended ? `0 0 30px ${color}40` : `0 0 20px rgba(0,0,0,0.5)`, transform: recommended ? 'scale(1.05)' : 'scale(1)', opacity: disabled ? 0.5 : 1, filter: disabled ? 'grayscale(100%)' : 'none' }}>
      <div>
      ...
      <div>
        ...
        <button onClick={disabled ? null : action} className="rune-btn" style={{ background: recommended ? color : 'transparent', border: `1px solid ${disabled ? '#444' : color}`, color: recommended ? '#000' : (disabled ? '#444' : color), width: '100%', padding: '20px', cursor: disabled ? 'not-allowed' : 'pointer', fontSize: '14px', letterSpacing: '2px', fontFamily: "'Oswald', sans-serif", fontWeight: 600, textTransform: 'uppercase' }} onMouseEnter={(e) => { if(!recommended && !disabled) { e.target.style.background = color; e.target.style.color = '#000' } }} onMouseLeave={(e) => { if(!recommended && !disabled) { e.target.style.background = 'transparent'; e.target.style.color = color } }}>{disabled ? 'LOCKED' : link}</button>
  ...
  return <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 200, backdropFilter: 'blur(20px)', background: 'rgba(0,0,0,0.7)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}><div onClick={onClose} style={{ position: 'absolute', top: '40px', right: '50px', cursor: 'pointer', color: '#fff', fontSize: '24px', opacity: 0.5 }}>✕ CLOSE</div><div style={{ marginBottom: '40px', textAlign: 'center' }}><RuneLogo size={60} /><h1 style={{ color: '#fff', fontFamily: "'Oswald', sans-serif", letterSpacing: '8px', marginTop: '20px', fontSize: '24px', fontWeight: 300 }}>ACQUISITION PROTOCOL</h1></div><div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center' }}><Card title="PROTOCOL 1 // SOURCE" price={data.licensePrice} color="#00ffea" link="INITIATE DOWNLOAD" action={() => window.open(data.stripeLink, '_blank')} features={['REACT COMPONENT', 'GLSL SOURCE CODE', 'COMMERCIAL LICENSE', 'SELF-HOSTED']} disabled={isModified} warning={isModified ? "CANNOT BUY SOURCE FOR CUSTOM ASSET" : null} /><Card title="PROTOCOL 2 // DEPLOY" price="$1,499" color="#fff" link={isModified ? "DEPLOY CONFIGURATION" : "START CONFIGURATION"} recommended={true} action={onConfigTier2} features={['HOSTED SOLUTION (VERCEL)', 'YOUR CUSTOM CONFIG', 'CUSTOM DOMAIN SETUP', '1 REVISION ROUND']} /><Card title="PROTOCOL 3 // AGENCY" price="$3,800+" color="#ff0055" link="BUILD SCOPE" action={onConfigTier3} features={['FULL SITE ARCHITECTURE', 'CMS & COMMERCE ADD-ONS', 'SEO & ANALYTICS SUITE', 'DEDICATED SUPPORT']} /></div></div>
}

const ConfigTier2 = ({ mode, onClose, activeColor, setColor, activeText, setText, discountApplied }) => {
  ...
  const handleSubmit = () => {
    ...
    const subject = `RUNE DEPLOYMENT: ${mode} ${discountApplied ? '(DISCOUNT APPLIED)' : ''}`
    const body = generatePacket()
    window.location.href = `mailto:quotes@runedigitalstudio.com?subject=${subject}&body=${encodeURIComponent(body)}`
    
    // 2. SHOW THE MANUAL TERMINAL (In case #1 fails)
    setSubmitted(true)
  }
  ...
  if (submitted) {
    return (
      <div style={{ position: 'fixed', top: 0, right: 0, width: '500px', height: '100%', background: 'rgba(5,5,5,0.98)', borderLeft: '1px solid #333', zIndex: 300, padding: '60px', display: 'flex', flexDirection: 'column' }}>
        <div onClick={onClose} style={{ position: 'absolute', top: '40px', left: '-60px', cursor: 'pointer', color: '#fff', fontSize: '24px', opacity: 0.5 }}>✕</div>
        
        <h1 style={{ color: '#00ffea', fontSize: '36px', margin: '0 0 20px 0', fontWeight: 500 }}>TRANSMISSION LOG</h1>
        <p style={{ color: '#888', fontSize: '14px', lineHeight: '1.6', marginBottom: '30px' }}>
          WE HAVE ATTEMPTED TO LAUNCH YOUR EMAIL CLIENT.
          <br/><br/>
          <strong style={{ color: '#fff' }}>DID NOTHING HAPPEN?</strong>
          <br/>
          COPY THE DATA PACKET BELOW AND EMAIL IT DIRECTLY TO:
          <br/>
          <span style={{ color: '#ff0055', textDecoration: 'underline' }}>quotes@runedigitalstudio.com</span>
        </p>

        <textarea 
          readOnly 
          value={generatePacket()} 
          style={{ 
            width: '100%', height: '300px', background: '#111', 
            border: '1px solid #333', color: '#0f0', fontFamily: 'monospace', 
            fontSize: '12px', padding: '15px', resize: 'none', outline: 'none' 
          }} 
        />
        
        <button 
          onClick={() => { navigator.clipboard.writeText(generatePacket()); alert("DATA COPIED TO CLIPBOARD") }}
          className="rune-btn" 
          style={{ 
            marginTop: '20px', background: '#333', color: '#fff', width: '100%', 
            padding: '15px', cursor: 'pointer', fontSize: '12px', letterSpacing: '2px', 
        ...
        <button 
          onClick={onClose}
          style={{ 
            marginTop: '20px', background: 'transparent', border: '1px solid #333', 
            color: '#666', width: '100%', padding: '15px', cursor: 'pointer', 
  ...
  return (
    <div style={{ position: 'fixed', top: 0, right: 0, width: '500px', height: '100%', background: 'rgba(10,10,10,0.95)', borderLeft: '1px solid #333', zIndex: 300, backdropFilter: 'blur(30px)', padding: '60px', display: 'flex', flexDirection: 'column', transform: 'translateX(0)' }}>
      ...
      <label style={{ color: '#888', fontSize: '12px', marginBottom: '10px', letterSpacing: '2px' }}>COMPANY NAME</label>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', gap: '15px', alignItems: 'flex-start' }}><input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} style={{ marginTop: '5px', cursor: 'pointer' }} /><div style={{ color: '#666', fontSize: '12px', lineHeight: '1.4' }}>I ACKNOWLEDGE THIS SERVICE IS A "AS-IS" DEPLOYMENT. INCLUDES 1 ROUND OF REVISIONS.</div></div>
      <div style={{ borderTop: '1px solid #333', paddingTop: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', color: '#fff', marginBottom: '20px', fontSize: '14px' }}>
          <span>DEPLOYMENT FEE</span>
          <span style={{ color: discountApplied ? '#00ffea' : '#fff' }}>
            {discountApplied ? <><span style={{textDecoration: 'line-through', color: '#555', marginRight: '10px'}}>$1,499</span>$1,199</> : '$1,499.00'}
          </span>
        </div>
        <button onClick={handleSubmit} className="rune-btn" style={{ background: agreed ? '#fff' : '#333', color: agreed ? '#000' : '#888', width: '100%', padding: '20px', cursor: agreed ? 'pointer' : 'not-allowed', fontSize: '14px', letterSpacing: '3px', fontFamily: "'Oswald', sans-serif", fontWeight: 600, textTransform: 'uppercase', border: 'none' }}>SUBMIT MANIFEST</button>
      </div>
    </div>
  )
}
const ConfigTier3 = ({ mode, onClose }) => {
  ...
  const handleSubmit = () => { const subject = `RUNE AGENCY INQUIRY: ${mode}`; const body = JSON.stringify({ tier: 'PROTOCOL 3', asset: mode, addons, est_price: finalPrice }, null, 2); window.location.href = `mailto:quotes@runedigitalstudio.com?subject=${subject}&body=${encodeURIComponent(body)}` }
  return <div style={{ position: 'fixed', top: 0, right: 0, width: '500px', height: '100%', background: 'rgba(10,10,10,0.95)', borderLeft: '1px solid #333', zIndex: 300, backdropFilter: 'blur(30px)', padding: '60px', display: 'flex', flexDirection: 'column', transform: 'translateX(0)' }}><div onClick={onClose} style={{ position: 'absolute', top: '40px', left: '-60px', cursor: 'pointer', color: '#fff', fontSize: '24px', opacity: 0.5 }}>✕</div><h3 style={{ color: '#ff0055', fontSize: '14px', letterSpacing: '3px', marginBottom: '10px' }}>PROTOCOL 3 // AGENCY</h3><h1 style={{ color: '#fff', fontSize: '36px', margin: '0 0 40px 0', fontWeight: 500 }}>SCOPE BUILDER</h1><div className="scope-item active" style={{ cursor: 'default' }}><div><div style={{color: '#fff', fontSize: '14px'}}>AGENCY CORE</div><div style={{color: '#666', fontSize: '10px'}}>ASSET + LANDING PAGE + DEPLOY</div></div><div style={{color: '#fff'}}>${basePrice}</div></div><div className={`scope-item ${addons.expansion ? 'active' : ''}`} onClick={() => setAddons({...addons, expansion: !addons.expansion})}><div><div style={{color: '#fff', fontSize: '14px'}}>EXPANSION PACK</div><div style={{color: '#666', fontSize: '10px'}}>ABOUT + TEAM + CONTACT + LEGAL PAGES</div></div><div style={{color: '#fff'}}>+$1,800</div></div><div className={`scope-item ${addons.cms ? 'active' : ''}`} onClick={() => setAddons({...addons, cms: !addons.cms})}><div><div style={{color: '#fff', fontSize: '14px'}}>DYNAMIC CMS</div><div style={{color: '#666', fontSize: '10px'}}>SELF-EDITABLE BLOG / NEWS / UPDATES</div></div><div style={{color: '#fff'}}>+$2,400</div></div><div className={`scope-item ${addons.commerce ? 'active' : ''}`} onClick={() => setAddons({...addons, commerce: !addons.commerce})}><div><div style={{color: '#fff', fontSize: '14px'}}>COMMERCE ENGINE</div><div style={{color: '#666', fontSize: '10px'}}>STRIPE / SHOPIFY INTEGRATION</div></div><div style={{color: '#fff'}}>+$3,000</div></div><div onClick={toggleAll} style={{ textAlign: 'center', padding: '10px', border: '1px dashed #555', color: '#888', fontSize: '10px', letterSpacing: '2px', cursor: 'pointer', marginTop: '20px' }}>[ {isFullSuite ? 'RESET SELECTION' : 'SELECT FULL SUITE (SAVE 10%)'} ]</div><div style={{ flexGrow: 1 }} /><div style={{ borderTop: '1px solid #333', paddingTop: '30px' }}><div style={{ display: 'flex', justifyContent: 'space-between', color: '#fff', marginBottom: '5px', fontSize: '14px' }}><span>ESTIMATED TOTAL</span><span style={{ textDecoration: discountActive ? 'line-through' : 'none', color: discountActive ? '#888' : '#fff' }}>${total.toLocaleString()}</span></div>{discountActive && (<div style={{ display: 'flex', justifyContent: 'space-between', color: '#00ffea', marginBottom: '20px', fontSize: '18px', fontWeight: 'bold' }}><span>BUNDLE PRICE</span><span>${finalPrice.toLocaleString()}</span></div>)}<button onClick={handleSubmit} className="rune-btn" style={{ background: '#ff0055', color: '#fff', width: '100%', padding: '20px', cursor: 'pointer', fontSize: '14px', letterSpacing: '3px', fontFamily: "'Oswald', sans-serif", fontWeight: 600, textTransform: 'uppercase', border: 'none', marginTop: discountActive ? '0' : '15px' }}>REQUEST PROPOSAL</button></div></div>
}
...
const SuccessScreen = ({ onClose }) => {
  ...
  return <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 500, background: 'rgba(5,5,5,0.95)', backdropFilter: 'blur(20px)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}><RuneLogo size={100} /><h1 style={{ color: '#00ffea', fontFamily: "'Oswald', sans-serif", fontSize: '48px', letterSpacing: '5px', marginTop: '40px', textTransform: 'uppercase' }}>ASSET SECURED</h1><div style={{ color: '#fff', fontFamily: "'Oswald', sans-serif", fontSize: '16px', letterSpacing: '2px', marginTop: '10px', textAlign: 'center', maxWidth: '400px', lineHeight: '1.6' }}>PAYMENT VERIFIED. SOURCE CODE DISPATCHED TO YOUR EMAIL.</div><div style={{ height: '1px', width: '300px', background: '#333', margin: '40px 0' }} />{!joined ? (<div style={{ width: '100%', maxWidth: '400px', textAlign: 'center' }}><h2 style={{ color: '#888', fontSize: '14px', letterSpacing: '3px', marginBottom: '20px' }}>INITIALIZE SUPPLY DROP?</h2><div style={{ display: 'flex', gap: '10px' }}><input className="rune-input" placeholder="ENTER EMAIL" value={email} onChange={(e) => setEmail(e.target.value)} style={{ marginBottom: 0 }} /><button onClick={handleJoin} className="rune-btn" style={{ background: '#ff0055', color: '#fff', border: 'none', padding: '0 30px', fontFamily: "'Oswald'", fontWeight: 600, cursor: 'pointer' }}>JOIN</button></div></div>) : (<div style={{ color: '#ff0055', fontSize: '18px', letterSpacing: '3px', fontWeight: 600 }}>// BLACKLIST ACCESS CONFIRMED</div>)}<button onClick={onClose} style={{ marginTop: '60px', background: 'transparent', border: '1px solid #333', color: '#666', padding: '15px 40px', fontFamily: "'Oswald'", letterSpacing: '2px', cursor: 'pointer' }} onMouseEnter={(e) => {e.target.style.borderColor = '#fff'; e.target.style.color = '#fff'}} onMouseLeave={(e) => {e.target.style.borderColor = '#333'; e.target.style.color = '#666'}}>RETURN TO SHOWROOM</button></div>
}
const ControlDeck = ({ activeColor, setColor, activeText, setText, activeFont, setFont, defaultColor, defaultText, isModified }) => {
  return (
    <div style={{ 
      ...
      >
        {/* Ensure FONTS is defined at the top of your file! */}
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <input type="color" value={activeColor} onChange={(e) => setColor(e.target.value)} style={{ height: '45px', width: '45px', border: '1px solid #333', background: 'none', cursor: 'pointer', padding: 0 }} />
      ...
      {isModified && (
        <div onClick={() => { setColor(defaultColor); setText(defaultText); setFont('INDUSTRIAL'); }} style={{ textAlign: 'center', fontSize: '10px', color: '#888', letterSpacing: '1px', cursor: 'pointer', borderBottom: '1px dashed #555', paddingBottom: '2px', display: 'inline-block', width: '100%' }}>// REVERT TO FACTORY DEFAULTS</div>
      )}
    </div>
  )
}
...
// --- INTERVENTION MODAL (THE DOWNSELL TRAP) ---
const InterventionModal = ({ onClose, onReset, onClaimOffer }) => (
  <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 600, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
    <div style={{ width: '400px', border: '1px solid #ff0055', background: '#000', padding: '40px', textAlign: 'center', position: 'relative' }}>
      ...
      <p style={{ color: '#888', fontSize: '14px', lineHeight: '1.6', marginBottom: '30px' }}>
        YOU ARE ABOUT TO DESTROY YOUR CUSTOM CONFIGURATION.
        <br/><br/>
        WE CAN DEPLOY YOUR CUSTOM ASSET FOR A REDUCED PRICE.
      </p>
      ...
      <button 
        ...
        style={{ 
          ...
          border: 'none', padding: '20px', fontWeight: 'bold', 
          marginBottom: '15px', cursor: 'pointer', letterSpacing: '2px'
        }}
      >
        CLAIM 20% OFF ($1,199)
      </button>
      ...
      >
        NO, DELETE MY WORK & RESET TO DEFAULT
      </button>
    </div>
  </div>
)
