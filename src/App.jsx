   ...
   <Canvas camera={{ position: [0, 1, isMobile ? 16 : 12], fov: 50 }}>
    ...
    <Float speed={2} rotationIntensity={0.1} floatIntensity={0.2}>
     <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -3, 0]}>
      ...
      <MeshDistortMaterial color="#000000" roughness={0} metalness={1} distort={0.3} speed={1.5} />
     </mesh>
    </Float>
    <Sparkles count={300} scale={[20, 15, 10]} size={3} speed={0.4} opacity={0.8} color="#ffaa00" noise={0.5} />
    <Float speed={0.5} rotationIntensity={0.05} floatIntensity={0.2}>
     <group onClick={() => setWarping(true)} position={[0, 1, 0]}>
      <Text font={fontUrl} fontSize={isMobile ? 1.2 : 1.8} letterSpacing={0.15} color="white" anchorX="center" anchorY="middle">
       RUNE DIGITAL
       <meshStandardMaterial color="white" emissive="#ffaa00" emissiveIntensity={warping ? 4 : 0.2} toneMapped={false} />
      </Text>
      <Text font={fontUrl} position={[0, -1.2, 0]} fontSize={0.15} letterSpacing={0.3} color="#666" anchorX="center">
       PRESS TO INITIALIZE LINK
       <meshBasicMaterial color={warping ? "#ffaa00" : "#666"} />
      </Text>
      <mesh visible={false}><planeGeometry args={[10, 5]} /></mesh>
     </group>
    </Float>
...
const AcquisitionMatrix = ({ mode, onClose, onConfigTier2, onConfigTier3, isModified, onResetRequest, isMobile }) => {
 ...
 const Card = ({ title, price, features, color, link, action, recommended, disabled, warning }) => (
  <div style={{ border: `1px solid ${disabled ? '#333' : color}`, background: recommended ? 'rgba(20,20,20,0.95)' : 'rgba(0,0,0,0.95)', padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: isMobile ? 'auto' : '480px', width: isMobile ? '100%' : '320px', marginBottom: isMobile ? '20px' : '0', transition: 'all 0.3s ease', boxShadow: recommended ? `0 0 30px ${color}40` : `0 0 20px rgba(0,0,0,0.5)`, transform: (!isMobile && recommended) ? 'scale(1.05)' : 'scale(1)', opacity: disabled ? 0.5 : 1, filter: disabled ? 'grayscale(100%)' : 'none' }}>
   <div>
...
const ControlDeck = ({ activeColor, setColor, activeText, setText, activeFont, setFont, defaultColor, defaultText, isModified, isMobile }) => {
 ...
 if (isMobile) {
  if (!isOpen) {
   return <div onClick={() => setIsOpen(true)} style={{ position: 'fixed', bottom: '20px', left: '20px', right: '20px', background: 'rgba(20,20,20,0.9)', border: '1px solid #333', padding: '15px', zIndex: 100, color: '#fff', textAlign: 'center', fontFamily: "'Oswald'", letterSpacing: '2px', fontSize: '12px', cursor: 'pointer' }}>/// OPEN CONTROLS ///</div>
  }
  return (
 ...
 return (
  <>
   ...
   <div style={{ opacity: (viewMode === 'IDLE') ? 1 : 0, transition: 'opacity 0.5s ease', pointerEvents: (viewMode === 'IDLE') ? 'auto' : 'none', position: 'relative', zIndex: 10 }}>
    ...
    <ControlDeck activeColor={activeColor} setColor={setActiveColor} activeText={activeText} setText={setActiveText} activeFont={activeFont} setFont={setActiveFont} defaultColor={defaultColor} defaultText={defaultText} isModified={isModified} isMobile={isMobile} />
   </div>
